...
Rule Name:
	KMS_KEYS_TO_NOT_DELETE

Description:
	Check that Customer Managed keys are not scheduled for deletion. The rule is NON_COMPLAINT if the Customer Managed Keys are scheduled for deletion. This rule does not include AWS-managed and Imported Keys.
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    if 'kmsKeyIds' in valid_rule_parameters:
        for key_id in valid_rule_parameters['kmsKeyIds']:
            if key_id in all_kms_key_list:
                kms_key_details = kms_client.describe_key(KeyId=key_id)
                if kms_key_details['KeyMetadata']['Origin'] == 'AWS_KMS' and kms_key_details['KeyMetadata']['KeyManager'] == 'CUSTOMER':
                ...
                evaluations.append(build_evaluation(key_id, 'NOT_APPLICABLE', event, annotation='The given kmsKeyId does not exist. Please verify the kmsKeyId and try again.'))
...
def evaluate_parameters(rule_parameters):
    if rule_parameters:
        kms_key_list = rule_parameters['kmsKeyIds'].replace(" ", "")
        kms_key_list = kms_key_list.split(',')

        regex_pattern = re.compile("^[a-zA-Z0-9]{8}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{12}$")
        for kms_key in kms_key_list:
            if not regex_pattern.match(kms_key):
                raise ValueError('The KMS Key id should be in the right format.')
        rule_parameters['kmsKeyIds'] = kms_key_list
